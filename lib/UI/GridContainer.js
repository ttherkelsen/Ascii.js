...
class GridContainer extends Container {
	...
	lw = lh = 0;
        let cols = Array(me.columns).fill(0);
        let rows = Array(me.rows).fill(0);
        
        // Find max width/height for each column/row
        for (let [ idx, [ w, h ] ] of _layout.entries()) {
            if (w > cols[idx % me.columns]) {
                cols[idx % me.columns] = w;
            }
            if (h > rows[Math.trunc(idx / me.columns)]) {
                rows[Math.trunc(idx / me.columns)] = h;
            }
        }

        // FIXME: If me.normalise is true, make every col/row as large as the largest col/row

        // Add padding to the right/top of every cell
        // Add lines (horizontal/vertical) if headers are specified
